...
function OnPopup( popupInfo )
	...
	if( popupInfo.Type ~= ButtonPopupTypes.BUTTONPOPUP_WHOS_WINNING ) or (getBRDataOption("WhosWinning")== 1) then
		if (getBRDataOption("WhosWinning")== 1) and (popupInfo.Type == ButtonPopupTypes.BUTTONPOPUP_WHOS_WINNING) then
			logger:debug("Whos Winning PopUp Suppressed");
		end;
		return;
	end
