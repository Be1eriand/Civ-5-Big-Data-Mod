...
function OnPopup( popupInfo )
	...
	if( popupInfo.Type ~= ButtonPopupTypes.BUTTONPOPUP_LEAGUE_SPLASH ) or (getBRDataOption("WorldCongress")== 1) then
		if (getBRDataOption("WorldCongress")== 1) and (popupInfo.Type == ButtonPopupTypes.BUTTONPOPUP_LEAGUE_SPLASH) then
			logger:debug("World Congress Splash Suppressed");
		end;		
		return;
	end
